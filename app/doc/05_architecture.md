...
The Focus preview is not a standalone daemon: `CameraStream` runs in a thread inside the single uWSGI web worker. Picamera2 is configured for a fixed 10 FPS by default using `FrameDurationLimits`, and `queue=False` requires each `capture_array()` call to wait for a fresh completed request. The application keeps only one latest JPEG in memory; it does not maintain a frame backlog. Flask and nginx both disable caching/buffering for the MJPEG route. Worker-local telemetry reports the configured FPS, rolling measured FPS, frame count, and newest-frame age without writing every frame to disk.

`CameraStream` records lifecycle transitions (`starting`, `running`, `stalled`, `error`, `stopped`, `worker_terminating`) in worker-local state and the shared scheduler status. Camera-driver exceptions are propagated to the stream owner so the camera health, UI, hardware lock, and logs all reach a terminal state together.

Recovery is bounded. After `STREAM_STALL_TIMEOUT` without a frame, the watchdog records resource data and all Python thread stacks, then attempts `Picamera2.close()` in a daemon helper so a blocked native close cannot block the watchdog. If the stream has not unwound by `STREAM_CLOSE_GRACE_TIMEOUT`, the watchdog verifies that the thread and generation are still current and sends `SIGKILL` only to that uWSGI web worker. The uWSGI master respawns it. This is not a Raspberry Pi reboot and does not stop the scheduler mule, which is a separate process. Outside a verified uWSGI worker, automatic process termination is refused. If the master does not replace the worker, restart the uWSGI service after collecting the SHDL, crash, uWSGI, and kernel journals.
...
* **Group-writable copies:** rclone is run with `--no-update-dir-modtime` so shared destinations do not fail when another user owns a directory. Local copies use umask `002` (new files `664`, dirs `775`). SSH/SFTP cannot inherit the Pi’s umask, so after copy the daemon chmods only the files that run created or updated (not a recursive walk of a 20–30 GB tree). FTP and cloud remotes are copy-only.
