...
* **Software Update:** Safely pulls the latest ChronoRoot code in `/srv/ChronoRootControl`. If local files or commits block the pull, the page asks before forcing the device to match its remote branch. Confirming permanently discards tracked changes, local commits, and untracked files; ignored files are preserved. Repository ownership mismatches are handled automatically. Restart the services or reboot afterwards to run the new version. The same two-step action is exposed to the Fleet Commander via `POST /api/update`.
...
* **Group access:** Local copies are created group-writable (umask `002`). SSH/SFTP uploads follow the server umask; prepare the destination once as `chgrp labgroup /path && chmod 2775 /path`. FTP and cloud destinations are unchanged.
